 ...
 *
 * All methods accepting a number as a parameter accept either a BigInteger instance, an integer, or a string
 * representing an arbitrary size integer.
 *
 * @psalm-immutable
 */
final class BigInteger extends BigNumber
{
     ...
     */
    public static function randomBits(int $numBits, ?callable $randomBytesGenerator = null): self
    {
        if ($numBits < 0) {
     ...
     */
    public static function randomRange($min, $max, ?callable $randomBytesGenerator = null): self
    {
        $min = self::of($min);
        $max = self::of($max);
     ...
     * @return BigInteger The result.
     */
    public function dividedBy($that, int $roundingMode = RoundingMode::UNNECESSARY): self
    {
        $that = self::of($that);
     ...
     */
    public function modPow($exp, $mod): self
    {
        $exp = self::of($exp);
        $mod = self::of($mod);
     ...
     * @param BigNumber|int|float|string $that The operand. Must be convertible to an integer number.
     */
    public function and($that): self
    {
        $that = self::of($that);

        return new self(Calculator::get()->and($this->value, $that->value));
    }
     ...
     * @param BigNumber|int|float|string $that The operand. Must be convertible to an integer number.
     */
    public function or($that): self
    {
        $that = self::of($that);

        return new self(Calculator::get()->or($this->value, $that->value));
    }
     ...
     * @param BigNumber|int|float|string $that The operand. Must be convertible to an integer number.
     */
    public function xor($that): self
    {
        $that = self::of($that);

        return new self(Calculator::get()->xor($this->value, $that->value));
    }
     ...
     */
    public function shiftedLeft(int $distance): self
    {
        if ($distance === 0) {
            return $this;
        }
        ...
        return $this->multipliedBy(self::of(2)->power($distance));
    }
     ...
     * @param int $distance The distance to shift.
     */
    public function shiftedRight(int $distance): self
    {
        if ($distance === 0) {
            return $this;
        }
        ...
        $operand = self::of(2)->power($distance);
    ...
    public function toBytes(bool $signed = true): string
    {
        if (! $signed && $this->isNegative()) {
            throw new NegativeNumberException(
                'Cannot convert a negative number to a byte string when $signed is false.'
            );
        }
